...
`,pe(n.championName,m,t);const B=document.createElement("div");B.className="item-container";for(let c=0;c<6;c++){const g=document.createElement("img");g.id=`ItemIcon-${m}_${c}`,g.alt=`Item ${c}`,B.appendChild(g)}v.appendChild(B),I.appendChild(v);for(let c=0;c<6;c++)await ge(n[`item${c}`],m,c,t);const w=document.createElement("div");w.className="SR-container";const D=document.createElement("div");D.className="summoner-container";for(let c=0;c<2;c++){const g=document.createElement("img");g.id=`SummonerIcon-${m}_${c}`,g.alt=`Summoner ${c+1}`,D.appendChild(g)}const x=document.createElement("div");x.className="rune-container";for(let c=0;c<2;c++){const g=document.createElement("img");g.id=`RuneIcon-${m}_${c}-${c===0?"main":"sub"}`,g.alt=`Rune ${c===0?"Main":"Sub"}`,x.appendChild(g)}w.appendChild(D),w.appendChild(x);const re=v.querySelector(".matchInfo-box"),ce=v.querySelector(".champProfileBox");re.insertBefore(w,ce.nextSibling);for(let c=0;c<2;c++)await Ce(n[`summoner${c+1}Id`],m,c,t);const le=n.perks.styles[0].selections[0].perk,ie=n.perks.styles[1].style;await W(le,m,0,t),await W(ie,m,1,t),v.innerHTML+=`
      <div>
